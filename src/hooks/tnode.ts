...
import { getDefaultNode, getParams, getSlotFirst } from '../utils/renderNode'
...
function handleSlots(instance: ComponentInternalInstance, name: string, params: Record<string, any>) {
  ...
  if (node && node.filter(t => !isCommentVNode(t)).length)
    return node
  // 检查是否存在 中划线命名 的插槽
    ...
    if (isSlotFirst && renderSlot) {
      ...
      if (instance && isPropExplicitlySet(instance, name)) {
        ...
        if (isFunction(propsNode))
          return propsNode(h, renderParams)
 ...
 */
export function filterCommentNode(nodes: VNode[]): VNode[] {
